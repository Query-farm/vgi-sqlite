...
def test_attach_registers_both_functions_as_tables(
    ...
    assert {"row_transform_add_row_2", "row_transform_add_row_3", "row_transform_repeat_row"} <= names
...
def test_literal_call(conn: sqlite3.Connection, row_transform_worker_location: str) -> None:
    ...
    _attach(conn, row_transform_worker_location)
    assert conn.execute("SELECT * FROM row_transform_add_row_2(3, 4)").fetchall() == [(7,)]
...
def test_missing_argument_rejected_at_prepare_time(
    ...
    _attach(conn, row_transform_worker_location)
    try:
        conn.execute("SELECT * FROM row_transform_add_row_2(3)").fetchall()
...
def test_correlated_call_one_row_per_input_row(
    ...
    rows = conn.execute(
        "SELECT pairs.x, pairs.y, t.sum FROM pairs, row_transform_add_row_2(pairs.x, pairs.y) AS t "
        "ORDER BY pairs.x"
    ).fetchall()
...
def test_arity_overload_gets_distinct_names(
    ...
    own name (`_<N>` suffix) and both are independently queryable and
    correct."""
    _attach(conn, row_transform_worker_location)
    assert conn.execute("SELECT * FROM row_transform_add_row_2(3, 4)").fetchall() == [(7,)]
    assert conn.execute("SELECT * FROM row_transform_add_row_3(3, 4, 5)").fetchall() == [(12,)]
